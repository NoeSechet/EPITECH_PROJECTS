...
int special_nb_words(char *line, char c)
{
    int word_count = 0;
...
char *special_fill_box(char *prog, int *parstr, char c)
{
    char *box = NULL;
    int len = *parstr;
    int length_box = 0;
...
{
    int nb_boxes = special_nb_words(str, c);
    char **tab = NULL;
    int parstr = 0;

    tab = malloc(sizeof(char *) * (nb_boxes + 1));
    for (int i = 0; i < nb_boxes; i += 1)
        tab[i] = special_fill_box(str, &parstr, c);
    tab[nb_boxes] = NULL;
    return (tab);
}