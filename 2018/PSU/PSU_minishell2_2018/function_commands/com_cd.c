...
void cd_with_path(char *path, list_t *list)
{
    ...
    if (comp_word_right("/home/noebrioche", get_environment_val("OLDPWD", list),
    my_strlen(OLD_PWD)) != 1 &&
    comp_word(path, "-", my_strlen(path)) == 1)
        chdir(get_environment_val("OLDPWD", list));
    else if (chdir(path) == -1 ||
    (comp_word_right(get_environment_val("PWD", list), get_environment_val("OLDPWD",
    list), my_strlen(OLD_PWD)) == 1 &&
